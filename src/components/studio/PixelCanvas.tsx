...
export default function PixelCanvas() {
  ...
  /**
   * Mede sempre a área útil do canvas, descontando a borda CSS.
   * Usar o rect completo (incluindo border) acumula erro de 1px e, em zoom
   * alto, faz a prévia parecer distante do cursor.
   */
  const canvasViewPoint = useCallback((e: React.PointerEvent) => {
    const canvas = canvasRef.current;
    if (!canvas) return null;
    const rect = canvas.getBoundingClientRect();
    const css = window.getComputedStyle(canvas);
    const left = Number.parseFloat(css.borderLeftWidth) || 0;
    const right = Number.parseFloat(css.borderRightWidth) || 0;
    const top = Number.parseFloat(css.borderTopWidth) || 0;
    const bottom = Number.parseFloat(css.borderBottomWidth) || 0;
    return {
  ...
  const cellFromEvent = useCallback((e: React.PointerEvent): [number, number] | null => {
    ...
    const point = canvasViewPoint(e);
    if (!point) return null;
    const x = cellFromView(0, point.x, point.width, project.width);
    const y = cellFromView(0, point.y, point.height, project.height);
    if (x === null || y === null) return null;
    return [x, y];
  }, [canvasViewPoint, project]);

  const posFromEvent = useCallback((e: React.PointerEvent): [number, number] | null => {
    if (!project) return null;
    const point = canvasViewPoint(e);
    if (!point) return null;
    const x = posFromView(0, point.x, point.width, project.width);
    const y = posFromView(0, point.y, point.height, project.height);
    if (x === null || y === null) return null;
    return [x, y];
  }, [canvasViewPoint, project]);
  ...
  const drawOverlayShape = useCallback((x1: number, y1: number, shiftKey: boolean, altKey: boolean) => {
    ...
    ctx.imageSmoothingEnabled = false;
    // O preview é um único quadro. Sem limpar aqui, cada movimento deixava
    // uma elipse antiga no overlay e dava a sensação de que a ferramenta
    // estava atrasada ou afastada do cursor.
