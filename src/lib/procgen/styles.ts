...
export type StyleId = 'fantasia' | 'scifi' | 'fofo' | 'sombrio' | 'retro' | 'natureza' | 'angola';
...
export const STYLE_PRESETS: Array<{ id: StyleId; label: string; desc: string }> = [
  ...
  { id: 'angola', label: 'Angola', desc: 'Terra, vermelho, ouro e verde' },
];
...
export function makeStyle(rng: Rng, style: StyleId): StyleSpec {
  ...
  switch (style) {
    ...
    case 'angola':
      return {
        metal: '#d9a441', metalDark: '#8b5f28', metalLight: '#f9d616',
        wood: '#7a4b2a', woodDark: '#3b2418',
        accent: '#ce1126', accentDark: '#8e0c1b',
        glow: '#f9d616', outline: '#171717',
        ramp: ['#8e0c1b', '#ce1126', '#f9d616'],
      };
    default: // fantasia
      return {
